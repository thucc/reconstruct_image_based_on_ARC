...
%===========================================================================================================%
	...
	PSNR							= zeros(1,6);
	mssim							= zeros(1,6);
	[PSNR(1),mssim(1),res_image1] = func_wiener_filter(spec_on_Dopt,ones(N),[num2str(ang),' directly restore on Dopt'],ang);
	[PSNR(2),mssim(2),res_image2] = func_wiener_filter(spec_on_Dvor,ones(N),[num2str(ang),' directly restore on Dvor'],ang);
	K1		      				= H./(H.^2.*(1+b.^2));
	[PSNR(3),mssim(3),res_image3] = func_wiener_filter(spec_on_Dopt,K1,[num2str(ang),' wiener filter on Dopt'],ang);
	[PSNR(4),mssim(4),res_image4] = func_wiener_filter(spec_on_Dvor,K1,[num2str(ang),' wiener filter on Dvor'],ang);
	K2	 	    				= H./(H.^2.*(1+a.^2+b.^2));
	[PSNR(5),mssim(5),res_image5] = func_wiener_filter(spec_on_Dopt,K2,[num2str(ang),' wiener filter consider alias on Dopt'],ang);
	[PSNR(6),mssim(6),res_image6] = func_wiener_filter(spec_on_Dvor,K2,[num2str(ang),' wiener filter consider alias on Dvor'],ang);
