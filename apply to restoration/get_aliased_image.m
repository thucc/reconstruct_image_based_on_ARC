...
%=======================================================获得原始图像的频谱==============================================
origin_hr_image 			= double(origin_hr_image);
origin_hr_spec 				= fftshift(fft2(origin_hr_image));
plot_spectrum(origin_hr_spec,'origin high resolution image''s spec');
...
figure;imshow(uint8(hr_image_after_MTF));title('image after MTF');
%(3)降采样
image_down_sample 			= hr_image_after_MTF(1:down_factor:N,1:down_factor:N);
figure;imshow(uint8(image_down_sample));title('image after MTF and down sampling')
%(4)加噪声
sigma 						= params.sigma;
